...
export default function Game({ gameMode }: Readonly<GameProps>) {
	...
	const [autoSkipProgress, setAutoSkipProgress] = useState<number | null>(null);
	...
	const handleSkip = () => {
		...
		// (in Training, the skip button state is handled by handleContinue)
	...
	// On TRAINING mode, the skip button trigger the continue function
	// The game shows the correct answer for 3s
	// Then skips automatically to the next question
	const handleContinue = () => {
		if (!gameMode.includes("TRAINING")) return;
		...
		// Disable the skip button during the animation
		setIsSkipping(true);

		const interval = setInterval(() => {
			currentStep += 1;
			setAutoSkipProgress(Math.min((currentStep / steps) * 100, 100));
		}, intervalMs);

		setTimeout(() => {
			clearInterval(interval);
			setAutoSkipProgress(null);
		...
		<div className="flex flex-col items-center justify-center p-2 md:p-6">
			<div className="w-full xl:max-w-xl mb-24">
				...
				{gameMode.includes("TRAINING") && (
					<div className="text-center text-gray-500 dark:text-gray-300 mb-4 text-xl">
				...
				<Card color1="yellow" color2="blue">
					<div ref={flagTopElement} className="flex items-center justify-between mb-8">
						...
						<Button
							...
							className={
								"text-secondary dark:text-primary bg-transparent hover:bg-transparent hover:text-gray-400 dark:hover:text-gray-600 focus:outline-none hover:shadow-none"
							}
