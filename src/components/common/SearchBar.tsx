...
export default function SearchBar({
	...
	useEffect(() => {
		...
		const splitWords = (text: string): string[] => normalize(text.toLowerCase()).split(/[\s\-_/]+/);
		...
		const wordStartsWithMatches = optionsArray.filter(
			(option) => !filtered.includes(option) && splitWords(option).some((word) => word.startsWith(lowerValue)),
		);

		// Then: match where input is included anywhere (but not already matched)
		const includesMatches = optionsArray.filter(
			(option) =>
		...
		const firstMatch = combined.find((option) => normalize(option.toLowerCase()).startsWith(lowerValue));
